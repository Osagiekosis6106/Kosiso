# XF-85 Goblin: fact-check log

Checked 25 Sep 2026 against the script the user is editing, "Why Did This Fighter Have No Landing Gear?". ✅ confirmed · ✏️ fix the wording · ⚠️ check a photo before using.

## Sources to open (to confirm anything yourself)

| # | Source | Link |
|---|---|---|
| S1 | National Museum of the US Air Force: XF-85 fact sheet | https://www.nationalmuseum.af.mil/Visit/Museum-Exhibits/Fact-Sheets/Display/Article/195800/mcdonnell-xf-85-goblin/ |
| S2 | Air Force Test Center: "August 23, 1948: XF-85 Goblin Aircraft Debuts" | https://www.aftc.af.mil/About-Us/History/On-This-Day-in-Test-History/Article-Display-Test-History/Article/2285389/august-23-1948-xf-85-goblin-aircraft-debuts/ |
| S3 | Defense Media Network: "The XF-85 Goblin" | https://www.defensemedianetwork.com/stories/the-xf-85-goblin-the-parasite-fighter-that-didnt-work/ |
| S4 | SAC & Aerospace Museum (Nebraska): XF-85 exhibit | https://www.sacmuseum.org/visit/exhibit/xf-85-goblin/ |
| S5 | Vintage Aviation News: "Grounded Dreams: XF-85 Goblin" | https://vintageaviationnews.com/warbird-articles/grounded-dreams/xf-85-goblin.html |
| S6 | The Aviationist: "A Parasitic Goblin" | https://theaviationist.com/2024/09/16/a-parasitic-goblin-the-story-of-the-xf-85/ |
| S7 | Wikipedia: McDonnell XF-85 Goblin (specs box and references) | https://en.wikipedia.org/wiki/McDonnell_XF-85_Goblin |
| S8 | Wikipedia: Zveno project | https://en.wikipedia.org/wiki/Zveno_project |
| S9 | Wikipedia: FICON project | https://en.wikipedia.org/wiki/FICON_project |
| S10 | John Weeks photo pages: both surviving Goblins on display | https://www.johnweeks.com/f85/xf85usafm.html · http://johnweeks.com/f85/xf85sac.html |

## Claim by claim

| Script line | Verdict | What the sources say |
|---|---|---|
| No landing gear, a hook on top of the nose | ✅ | Retractable hook engaging a trapeze (S1, S3) |
| "airfield… flying at over 200 miles an hour" | ✅ | The windblast came into the cockpit at about 200 mph (S2) |
| "probably the smallest jet fighter the US Air Force ever ordered" | ✅ ✏️ | The USAF museum says "the smallest American jet fighter ever built" (S1). You can drop "probably". |
| "barely longer than a car" **and** "shorter than most family cars" | ✏️ | These **contradict each other**. 14 ft 10 in = 4.52 m, about the length of a family car. Use "about the length of a family car" in both places. |
| "just under 4.5 metres long, 14 feet 10 inches" | ✏️ | 14 ft 10 in is **just over** 4.5 m (4.52 m). Change to "just over 4.5 metres". |
| B-36 wingspan ~70 m / 230 ft, wider than a 747 | ✅ | 230 ft; a 747's is 196–224 ft depending on version |
| Early jets could fly only an hour or two | ✅ | General fact; no specific figure is claimed |
| Navy airships caught biplane fighters on a trapeze in the 1930s | ✅ | Akron/Macon and Sparrowhawk. The Goblin's trapeze was based on theirs (S3). |
| Zveno flew real combat missions in 1941 | ✅ | Attacks on Constanța (26 Jul 1941) and the Chernavodă bridge (Aug 1941) (S8) |
| "one number: the size of a bomb bay" / "15 ft bay, about 2 inches to spare" | ⚠️ **Don't say it this way** | **Corrected 25 Sep.** Online sources give **15 ft as the design limit** for the fighter ("no longer than 15 feet, no taller than 9 feet, folded wings no wider than 5 feet"; [National Interest](https://nationalinterest.org/blog/buzz/the-us-air-forces-xf-85-goblin-the-scary-parasite-fighter-15671), [Plane Historia](https://planehistoria.com/mcdonnell-xf-85/)). **Not** as the length of the bay: one summary puts a B-36 bay at about 33 ft. Jenkins, *Magnesium Overcast*, is a print book and couldn't be checked online. Safe line: *"The Air Force gave McDonnell a box to fit: no more than about 15 feet long. The Goblin came in at 14 feet 10."* Or drop the number. Use "two inches to spare" **only** if Jenkins confirms a 15-ft limit. |
| Wings folded upward, span ~6.4 m / 21 ft | ✅ | 21 ft 1 in; swept wings folding upward (S1, S5) |
| Multiple small tail fins | ✅ | Three vertical fins plus angled horizontal surfaces (S1) |
| Westinghouse J34, ~3,000 lb thrust | ✅ | J34, 3,000 lbf |
| ~2 tonnes / 4,500 lb loaded | ✅ | 4,550 lb loaded (2.06 t), 3,740 lb empty |
| >1,000 km/h, ~650 mph | ✅ | 664 mph max on paper (≈1,069 km/h) |
| Four .50-calibre guns | ✅ | Four .50-cal machine guns (designed) |
| Ejection seat | ✅ | The pilot sat on the engine in an ejection seat |
| Retractable hook plus a steel emergency skid | ✅ | Skid landings are recorded on the failed flights (S2) |
| Tested on a modified B-29 because the B-36 wasn't available | ✅ | EB-29B, serial 44-84111 (S6) |
| Nicknamed "Monstro", after the whale that swallowed Pinocchio | ✅ | The Aviationist: "named Monstro, after the whale that swallowed Pinocchio" (S6); the SAC Museum exhibit page names the EB-29B "Monstro" (S4) |
| Test pilot Edwin Schoch; first free flight 23 Aug 1948 | ✅ | Edwin F. Schoch, released at 20,000 ft (S2) |
| Air under the bomber churns and pushes small aircraft away | ✅ | Turbulence under the EB-29 made recovery hard (S1, S2) |
| The bar hit the canopy, the glass shattered, windblast tore off helmet and oxygen mask | ✅ | (S2, S3) |
| Belly-skid landing on the dry lakebed; he survived | ✅ | Skid landing at Muroc (Rogers Dry Lake) (S2) |
| First successful hook-up "weeks later, in October 1948" | ✅ | 14 October 1948 (S6) |
| 7 free flights, only 3 hook-ups, the rest emergency landings | ✅ | 3 hook-ups, 4 belly landings, about 2 h 19 min in total, Aug 1948 – Apr 1949 (S3, S5) |
| Cancelled; aerial refuelling was the better answer | ✅ | Cancelled 24 Oct 1949; in-flight refuelling of escort jets replaced the concept (S5) |
| Only two built; one in Ohio, one in Nebraska | ✅ | 46-523 at the USAF Museum (Dayton, Ohio); 46-524 at the SAC & Aerospace Museum (Ashland, Nebraska) (S1, S4) |
| "The one in Ohio… hook still raised" | ✅ | Script now says only Ohio (the USAF Museum Goblin, on a stand with the hook deployed). Nebraska photo if wanted: https://www.airhistory.net/photo/88628/46-524 |
| "never once landed on [a runway] on purpose" | ✏️ | Every landing was a skid landing on the dry lakebed. "On purpose" suggests it once landed on a runway by accident. Say "and it never once landed on one." |
| 1950s: a full-size reconnaissance jet under a modified B-36 | ✅ | FICON: RF-84K under a GRB-36 (S9) |
| Today: big aircraft launching and catching drones | ✅ (generic) | If you **name** a programme (e.g. DARPA Gremlins), verify it first |
| Teases: tail-sitter / lopsided warplane / jet that hung on a wire | ✅ | Convair XFY Pogo · Blohm & Voss BV 141 · Ryan X-13 Vertijet |

## Reference-image checks (Grok masters)

| Slot | Image the user sent | Verdict | Notes |
|---|---|---|---|
| **R1** | Colour head-on photo of the USAF Museum Goblin (46-523) on its yellow display stand | ✅ **Chosen for R1** | Real XF-85, sharp and symmetrical. Shows the nose intake, bubble canopy, **hook raised**, swept wings with wingtip fins, upper tail fins and USAF roundel. It also visually confirms the script's line "the Goblin in Ohio sits on a stand, its hook still raised". Wings are **spread**, not folded, so M1 should be generated wings-spread (safer than asking AI to invent the fold). Crop out the stand, curtains, electrical box, sign and rope before uploading. Credit: National Museum of the US Air Force (US government photo, public domain; confirm on the source page). |
| **R1 (final)** | Same museum photo, cleaned: stand, curtains and clutter removed, plain dark studio background | ✅ **Use this as R1** | Aircraft geometry unchanged from the original: nose intake, canopy, raised hook, both upper tail fins, wingtip fins, roundel all intact. Stand post under the nose now painted out cleanly (checked 25 Sep). Remaining note: no floor shadow, so the plane floats. In the M1 prompt add "hovering just above the studio floor, no stand, no wheels" so Grok doesn't give it landing gear to stand on. Keep the unedited original as your proof photo. |
| R1 (alt) | B&W 1948 overhead photo of the Goblin on its ground dolly | ⚠️ Keep for archival B-roll, not for R1 | Real XF-85 period photo (hook up, "NO STEP" markings, wingtip fins). But it's top-down, not front/front-3/4, low resolution (800 px), and it shows the **wheeled ground dolly**, so Grok would likely add wheels. Use it in the edit as an `[ARCH]` shot, e.g. behind "no wheels at all" with a caption that the wheels are a ground dolly. |
| **M1 try 1** | Two AI renders (made in Google's image tool) | ❌ Regenerate or edit | **Hook is wrong in both:** a tiny ring or eyelet on the nose instead of the tall hook frame rising above the canopy (R1). That's the key feature of the whole video. Also: render 1 has insignia on **both** wings (the real aircraft shows only one on the upper surface); both lack the dark anti-glare panel on the nose. Correct in both: round nose intake, two upswept tail fins, wingtip fins, no wheels, hovering. **Render 2 is the better base** (single insignia, like R1). Fix the hook with an edit, or regenerate with the updated `[GOBLIN LOCK]`. |
| **M1 try 2** | AI render with the updated prompt | ❌ Worse overall | The hook is now a giant thin shepherd's crook, far too tall and the wrong shape (the real one is a short, sturdy, dark boxy frame just above the canopy). **The two upswept tail fins disappeared.** Insignia is on the wrong wing (mirror of R1). Improved: dark nose panel ✓. Lesson: full regenerations keep swapping one error for another, so fix try-1 render 2 with a targeted hook-only edit instead. |
